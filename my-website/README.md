# Portfolio — Project Architecture (2026)

## Overview

This repository contains a small Go backend and an Angular frontend that together form the `Portfolio` example project. The codebase demonstrates several concurrency patterns, worker-pool implementations, and a simple web frontend to consume backend APIs.
...
```bash
go build -o portfolio-server ./cmd
.
\portfolio-server.exe
```
